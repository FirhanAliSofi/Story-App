...
import com.firhanalisofi.submissionstoryapp.utils.MainDispatcherRule
...
class MainViewModelTest {
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun `when Get Story Should Not Null and Success`() =
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun `when Get Story Should Not Null and Empty`() =
        mainDispatcherRules.runBlockingTest {
            val mData: PagingData<ListStoryItem> = PagingSourceStory.snapshot(emptyList())
            val expectedResponse = MutableLiveData<PagingData<ListStoryItem>>()
...
class PagingSourceStory : PagingSource<Int, ListStoryItem>() {
    companion object {
